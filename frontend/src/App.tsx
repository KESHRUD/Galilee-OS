...
import { Search, Wifi, WifiOff, Box, Settings, LogOut, PlusCircle, Layout, Brain, Volume2, VolumeX, Languages } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
        // Seeding IndexedDB for Flashcards/Columns/Tasks
  ...
  const handleSaveTask = async (taskData: Partial<Task>) => {
    ...
    if (taskData.id) {
        ...
        newTask = {
            ...
            diagramCode: taskData.diagramCode
        };
    }
  ...
  const handleDeleteTask = async (id: string) => {
    if (confirm(t('confirm_action'))) {
        ...
        audioManager.play('error', theme); // Deletion sound
    }
  };
  ...
  const handleAddColumn = async () => {
      const name = prompt(t('add_column') + ":");
      if (!name) return;
    ...
    <div className={`h-screen w-full flex flex-col overflow-hidden relative`}>
      ...
      <header className={`${isGalilee ? 'bg-slate-900/60 border-b border-cyan-900/30' : 'bg-white/60 border-b border-slate-200/50'} backdrop-blur-md px-6 py-3 flex justify-between items-center z-20 shadow-sm transition-colors duration-500`}>
        ...
        <div className="flex items-center gap-4">
            <div className={`flex items-center gap-3 text-[10px] ${isGalilee ? 'text-cyan-600 font-mono' : 'text-slate-500'}`}>
            ...
            <div className="h-full flex flex-col">
                ...
                <div className="flex-1 overflow-x-auto overflow-y-hidden custom-scrollbar">
                    <div className="h-full flex p-6 min-w-max items-start">
                        ...
                        <div className="w-80 h-full flex-shrink-0 flex items-start justify-center pr-6">
                            <button 
